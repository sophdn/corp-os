...
Tools for the research step:

- `WebSearch` — find candidates ("python websocket streaming libraries 2025")
- `WebFetch` — read the actual docs (returns markdown)
- `Bash`: `pip show websockets | grep Version` — check what's installed in the project's venv

For libraries without docs pages, clone and read their `README.md` / `examples/` via `Read`. Context7 MCP (if configured) is also a good source — `mcp_*_resolve-library-id` then `mcp_*_query-docs`.
