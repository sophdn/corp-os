...
## When you're in a loaded role

If the session is in a loaded role (synthesist, code-auditor, daemon,
technowizard, white-hat, librarian, advisory-panel, prospector, role-keeper,
etc.), the role's craft notes live at `~/.claude/vault/roles/<role>/`.
Reflexes specific to a loaded-role context:

- **On load**, query `vault_search` with a question shaped like *"what
  patterns / failure modes / gotchas have I [the role] hit before?"* The
  search will surface entries from `roles/<role>/` if any exist; an empty
  result is fine (the role hasn't accumulated craft yet).
- **On observe**, when you notice a pattern worth keeping — an approach that
  worked, a failure mode to avoid, a craft technique — write it back via
  the file system to `~/.claude/vault/roles/<role>/<YYYY-MM-DD>_<slug>.md`.
  No MCP write surface; just edit the file. The next session in the same
  role will surface it via `vault_search`.
- **Cross-role craft** (patterns that apply to any role-loaded session,
  not just one) lives at `roles/general/`.
